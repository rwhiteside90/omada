...
# omada_ssl_import.sh
# Omada Controller SSL Certificate Import Script for Unix/Linux Systems
# by Rob Whiteside <http://www.robwhiteside.ca/>
# Written based off of  Unifi SSL Import script from Steve Jenkins
# Version 1.0 (Based off Steve Jenkins Version 2.8)
# Last Updated April 1, 2024

# REQUIREMENTS
# 1) Assumes you have a Omada Controller installed and running on your system.
...
# it's never a bad idea to manually back up your keystore (located at ${OMADA_DIR}/data/keystore/eap.keystore
# on Debian/Ubuntu systems) to a separate directory before running this
# script. If anything goes wrong, you can restore from your backup, restart the Omada Controller service,
...
if [[ ${LE_MODE} == "YES" || ${LE_MODE} == "yes" || ${LE_MODE} == "Y" || ${LE_MODE} == "y" || ${LE_MODE} == "TRUE" || ${LE_MODE} == "true" || ${LE_MODE} == "ENABLED" || ${LE_MODE} == "enabled" || ${LE_MODE} == 1 ]] ; then
        ...
        PRIV_KEY=${LE_LIVE_DIR}/${OMADA_HOSTNAME}/privkey.pem
        CHAIN_FILE=${LE_LIVE_DIR}/${OMADA_HOSTNAME}/fullchain.pem
else
        LE_MODE=false
...
if [[ ${LE_MODE} == "true" ]]; then
        ...
        if md5sum -c "${LE_LIVE_DIR}/${OMADA_HOSTNAME}/privkey.pem.md5" &>/dev/null; then
...
if [[ ${LE_MODE} == "true" ]]; then
        ...
        md5sum "${PRIV_KEY}" > "${LE_LIVE_DIR}/${OMADA_HOSTNAME}/privkey.pem.md5"
